...
IMPORTANT: 執行前、請勿在恢復期間重新開機儲存節點 `sn-recovery-postinstall.sh` （步驟4）重新格式化故障的儲存磁碟區、並還原物件中繼資料。重新啟動儲存節點 `sn-recovery-postinstall.sh` 完成會導致嘗試啟動服務時發生錯誤、並導致StorageGRID 不支援的應用裝置節點離開維護模式。
...
. 輸入以下命令、返回StorageGRID 到《The Monitor Install》（顯示器安裝）頁面 `\http://Controller_IP:8080`、使用運算控制器的IP位址。
+
「監控安裝」頁面會顯示指令碼執行時的安裝進度。



之後 `sn-recovery-postinstall.sh` 指令碼已在節點上啟動服務、您可以將物件資料還原至任何由指令碼格式化的儲存磁碟區、如下一步驟所述。
