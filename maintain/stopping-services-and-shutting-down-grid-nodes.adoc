...
summary: 在您關閉StorageGRID 某個VMware系統之前、您必須先停止每個網格節點上執行的所有服務、然後關閉所有VMware虛擬機器、Docker Container和StorageGRID VMware應用程式。 
---
= 停止服務並關閉網格節點
:allow-uri-read: 
:icons: font
...
[role="lead"]
在您關閉StorageGRID 某個VMware系統之前、您必須先停止每個網格節點上執行的所有服務、然後關閉所有VMware虛擬機器、Docker Container和StorageGRID VMware應用程式。

.關於這項工作
如果可能、您應該依照下列順序停止網格節點上的服務：
...
NOTE: 如果需要、您可以將節點從一部Linux主機移轉至另一部主機、以執行主機維護作業、而不會影響網格的功能或可用度。

link:linux-migrating-grid-node-to-new-host.html["Linux：將網格節點移轉至新主機"]
