...
IMPORTANT: 系統會提示您下載恢復套件檔案 (`sgws-recovery-package-id-revision.zip`）單擊Summary（摘要）頁面上的* Install*（安裝*）。您必須下載此檔案才能完成安裝。存取系統所需的密碼會儲存在中 `Passwords.txt` 檔案、包含在「恢復套件」檔案中。
