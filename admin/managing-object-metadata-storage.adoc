...
[cols="1a,1a,1a"]
|===
| 用於初始StorageGRID 安裝的版本 | 儲存節點上的RAM容量 | 預設的中繼資料保留空間設定StorageGRID 為適用於功能11.5 
...
在快照中、*實際保留*值為8 TB。此螢幕擷取畫面適用於全新StorageGRID 安裝的大規模儲存節點。由於此儲存節點的全系統中繼資料保留空間設定小於Volume 0、因此此節點的實際保留空間等於中繼資料保留空間設定。

*實際保留*值對應於此Prometheus指標：
...
假設您使用StorageGRID 11.5版安裝新的一套更新版的作業系統。在此範例中、假設每個儲存節點的RAM超過128 GB、而儲存節點1（SN1）的Volume 0為6 TB。根據這些值：

* 全系統*中繼資料保留空間*設定為8 TB。（StorageGRID 如果每個儲存節點的RAM超過128 GB、則此為新版的更新版的預設值。）
...
超過4 TB
 a| 
（中繼資料的實際保留空間：1 TB）x 60%、最高2.64 TB

|===

NOTE: 如果StorageGRID 您的系統在任何儲存節點上儲存（或預期會儲存）超過2.64 TB的中繼資料、在某些情況下、允許的中繼資料空間會增加。如果您的儲存節點各有超過128 GB的RAM、且儲存Volume 0上有可用空間、請聯絡您的NetApp客戶代表。NetApp將審查您的需求、並盡可能增加每個儲存節點的中繼資料空間。

若要檢視儲存節點允許的中繼資料空間：

. 從Grid Manager中選擇* Node_*>* Storage Node_*。
. 選擇* Storage*（儲存設備）選項卡。
...
假設您使用StorageGRID 版本11.5安裝一個還原系統。在此範例中、假設每個儲存節點的RAM超過128 GB、而儲存節點1（SN1）的Volume 0為6 TB。根據這些值：
...
.相關資訊
* 若要瞭解如何監控每個儲存節點的物件中繼資料容量：
+
link:../monitor/index.html["監控安培；疑難排解"]

* 若要增加系統的物件中繼資料容量、您必須新增儲存節點：
+
