...
.您需要的產品
在新增儲存磁碟區之前、請先檢閱新增儲存容量的準則、以確保您知道要在何處新增磁碟區、以符合ILM原則的要求。

link:adding-storage-capacity.html["增加儲存容量"]

[NOTE]
====
這些說明僅適用於軟體型儲存節點。請參閱SG6060應用裝置的安裝與維護說明、瞭解如何安裝擴充櫃、將儲存磁碟區新增至SG6060。無法擴充其他應用裝置儲存節點。
...
每個物件存放區都會掛載到與其ID相對應的磁碟區上。也就是說、ID為0000的物件存放區會對應到 `/var/local/rangedb/0` 掛載點。
