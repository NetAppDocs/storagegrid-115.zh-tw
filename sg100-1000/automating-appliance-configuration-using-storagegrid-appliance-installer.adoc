...
---
= 使用StorageGRID 《The Choice應用程式安裝程式：自動化應用裝置組態
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* 您必須使用StorageGRID 支援的瀏覽器、連線到您正在設定的應用裝置上的《The》（《The》）應用裝置安裝程式。
...
NOTE: 想要自動化設備安裝與組態的資深使用者、可以使用 `configure-sga.py` 指令碼：+link:automating-installation-configuration-appliance-nodes-configure-sga-py-script.html["使用configure-sga.py指令碼自動安裝及設定應用裝置節點"]
...
+
link:automating-installation-configuration-appliance-nodes-configure-sga-py-script.html["使用configure-sga.py指令碼自動安裝及設定應用裝置節點"]
