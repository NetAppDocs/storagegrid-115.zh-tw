...
NOTE: 如果您要將管理節點設備部署為節點複製目標、請在此停止部署程序、然後繼續執行節點複製程序以進行還原與維護。
...
.相關資訊
link:deploying-services-appliance-as-gateway-or-non-primary-admin-node.html["將服務應用裝置部署為閘道或非主要管理節點"]
