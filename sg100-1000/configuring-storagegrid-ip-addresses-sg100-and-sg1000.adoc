...
summary: 您可以使用StorageGRID 《支援服務的應用程式安裝程式」來設定StorageGRID 服務應用裝置在「支援網格」、「管理」和「用戶端網路」上所使用的IP位址和路由資訊。 
---
...
[role="lead"]
您可以使用StorageGRID 《支援服務的應用程式安裝程式」來設定StorageGRID 服務應用裝置在「支援網格」、「管理」和「用戶端網路」上所使用的IP位址和路由資訊。

.關於這項工作
您必須為每個連線網路上的應用裝置指派靜態IP、或為DHCP伺服器上的位址指派永久租期。

如果您要變更連結組態、請參閱變更服務應用裝置連結組態的指示。
