...
NOTE: 如果您要將儲存節點設備部署為節點複製目標、請在此停止部署程序、然後繼續執行節點複製程序以進行還原與維護。+link:../maintain/index.html["維護"]
...
NOTE: 如果您需要一次部署多個應用裝置儲存節點、可以使用將安裝程序自動化 `configure-sga.py` 應用裝置安裝指令碼。此指令碼僅適用於儲存節點。
